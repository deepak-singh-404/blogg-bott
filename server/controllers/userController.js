...
module.exports = {
    userRegister: async (req, res, next) => {
        try {
            ...
            await newUser.save()
            const payload = { id: newUser.id, user: {id:newUser.id, name: newUser.name, email: newUser.email, avatar: newUser.avatar } }
            jwt.sign(
                payload,
                keys.secretKey,
                { expiresIn: 7200 },
    ...
    userLogin: async (req, res, next) => {
        try {
            ...
            const payload = { id: user.id, user: { id: user.id, name: user.name, email: user.email, avatar: user.avatar}}
            jwt.sign(
                payload,
                keys.secretKey,
                { expiresIn: 7200 },
